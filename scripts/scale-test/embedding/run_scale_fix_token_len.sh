...
#   AUTO_ACTIVATE_SGLANG_CPU=1 ./run_scale_fix_token_len.sh ...
...
if [[ "$NOHUP_MODE" == "1" ]]; then
	# Determine scale_id (prefer user-provided).
	SCALE_ID=""
	RESULT_ROOT_IN=""
	args2=()
	while [[ $# -gt 0 ]]; do
...
PY
	...
	echo "[nohup] monitor: bash scripts/scale-test/embedding/monitor_scale_fix_token_len.sh --scale-id ${SCALE_ID} --result-root ${RESULT_ROOT}"
	exit 0
fi
