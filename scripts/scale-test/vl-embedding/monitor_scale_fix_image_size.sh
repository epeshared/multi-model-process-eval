...
usage() {
  cat <<'EOF'
Usage:
  monitor_scale_fix_image_size.sh --scale-id <SCALE_ID> [--result-root <DIR>] [--interval <SECONDS>]

Prints periodic status lines by inspecting per-host logs under the scale-test result directory.

Examples:
  bash scripts/scale-test/vl-embedding/monitor_scale_fix_image_size.sh --scale-id 20260228T031904Z
EOF
}

SCALE_ID=""
RESULT_ROOT="scripts/scale-test/vl-embedding/result/fix_image_size"
INTERVAL="30"
...
while true; do
  ...
  if pgrep -af "scripts/scale-test/vl-embedding/run_scale_fix_image_size\\.py" | grep -q "${SCALE_ID}"; then
    local_state="RUNNING"
  else
    # Fallback: shared runner path when invoked directly.
